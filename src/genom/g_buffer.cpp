 ...
 */
    ...
    GBuffer::~GBuffer() {
        unmap();
        vkDestroyBuffer(gDevice.device(), buffer, nullptr);
        vkFreeMemory(gDevice.device(), memory, nullptr);
    }
 ...
 */
    VkResult GBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
        ...
        if (size == VK_WHOLE_SIZE) {
            return vkMapMemory(gDevice.device(), memory, 0, bufferSize, 0, &mapped);
        }
        return vkMapMemory(gDevice.device(), memory, offset, size, 0, &mapped);
    }
 ...
 */
    VkResult GBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
        ...
        return vkFlushMappedMemoryRanges(gDevice.device(), 1, &mappedRange);
    }
 ...
 */
    VkResult GBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
        ...
        mappedRange.offset = offset;
        mappedRange.size = size;
        return vkInvalidateMappedMemoryRanges(gDevice.device(), 1, &mappedRange);
    }
 ...
 *
 */
    void GBuffer::writeToIndex(void *data, int index) {
